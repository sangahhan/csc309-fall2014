...
/*********************************************************************
 * 	GamePiece - Parent class of Bricks, Ball and the Paddle
 ********************************************************************/
...
/*********************************************************************
 * 	Ball - Has the ability to move all around the canvas
 ********************************************************************/
...
Ball.prototype.move = function () {
	// Move horizontally by xdirection units
	this.x += xdirection;

	// If the right edge of the ball hits the right wall or if the
	// left most point of the ball hits the the left ball
	if (this.x - this.width/2 <= 0 ||
		...
		// Have to change the xdirection if it hits an edge
...
/*********************************************************************
 * 	Paddle - Can move along the x coordinate
 ********************************************************************/
...
function testHitBricks() {
	...
	for (var i = 0; i < 4; i++){
		...
		if (index >= 0 && row < BRICK_ROWS && col < BRICK_COLS &&
			...
			// Increase the speed if it hits the orange row and if the
			// speed has not already been increased
			if ((row == 2 || row == 3) &&
			...
			// Increase the speed when hitting the red row for the first time
...
function movePaddle(evt) {
	if (evt.keyCode == 39) rightKeyPressed = true;
		...
		if (balls.length && !hasWon) {
			// There are still balls left and the person has not won.
			// If the user is playing and presses <Enter>, then it is
			// to pause the game, otherwise, it is to resume from pause.
			if (playing) gameStop();
			else {
				gameStart();
			...
			// If there are no more balls remaining or the person has won
			// Then the user will press enter to restart the whole game
			gameStop();
			hasWon = false;
			// back to beginning
			score = 0;
...
/*
 * Increase ball speed. Since we need the paddle to move
 * as fast as the ball, we increase the paddle speed as well.
 */
 ...
 * Move the ball to the next location if required.
...
/*
 * Clear the board, reset all values.
...
function levelCheck() {
	...
	if (!sum) {
		if (score == LEVEL_SCORE) {
			// Stop the game since we need to start the second level
			gameStop();

			// Reset the board leaving the current balls
			resetBoard(0);

			// Place the current ball on the paddle
			var x = (canvas.width / 2);
			var y = canvas.height - PADDLE_H - BALL_R;
			balls[0].x = x;
			balls[0].y = y;

			// draw all without moving the ball because we want it to
			// start moving after the user presses enter.
			drawAll(0);
			ballsSpan.html("LEVEL 2");
			return true;
		} else if (score == LEVEL_SCORE * 2) {
			// Stop because the player has won
			gameStop();

			// Draw a final time to make the last brick disappear
			drawAll(0);

			hasWon = true;
			score = 0;

			// Reset everything including the balls
			resetBoard(1);
...
function gameRun() {
	if (playing) {
		...
		// Check if the user hits the paddle. if he does, then
		// make sure the ball only goes upwards.
		if (!testHitPaddle()) {
			if (balls[0].y - balls[0].radius > (canvas.height - paddle.height)) {
				onLose();
				/*
		...
		// Check to see if a user has reached the next level or won.
		// If they haven't, then draw the game pieces and move the ball.
